...
namespace UnityEditor.ShaderGraph
{
    ...
    /// 
    /// * Note the name of the class still refers to the old project code-name
    /// </summary>
    public class WavesDefaultShaderGUI : ShaderGUI
    {
        [Flags]
        ...
        private static readonly ShaderConfig[] s_shaderConfigs =
        {
            ...
            new(
                "Waves/SG_WavesPBR_N_Underwater",
            ...
            new(
                "Waves/SG_WavesPBR_NS_Underwater",
            ...
            new(
                "Waves/SG_WavesPBR_FoliageUnderwater",
        ...
        public override void OnGUI(MaterialEditor editor, MaterialProperty[] properties)
        {
            ...
            {
                //Waves is using the metallic workflow, force it to metallic
            ...
            if (currentConfig.Type == ShaderType.Default)
            {
                GUILayout.Label("Waves Default PBR Shader", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.Normal)
            {
                GUILayout.Label("Waves PBR Shader with only Normal support", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.NormalSmooth)
            {
                GUILayout.Label("Waves PBR Shader with Smoothness support", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.NormalSmoothParallax)
            {
                GUILayout.Label("Waves PBR Shader with Smoothness and parallax support", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.NormalSmoothMetal)
            {
                GUILayout.Label("Waves PBR Shader with Smoothness and Metallic support", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.Foliage)
            {
                GUILayout.Label("Waves PBR Shader for foliage", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.UnderwaterNormal)
            {
                GUILayout.Label("Waves PBR Shader for underwater", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.UnderwaterNormalSmoothness)
            {
                GUILayout.Label("Waves PBR Shader for underwater with smoothness", EditorStyles.boldLabel);
            }

            if (currentConfig.Type == ShaderType.UnderwaterFoliage)
            {
                GUILayout.Label("Waves PBR Shader for underwater foliage", EditorStyles.boldLabel);
            }
            ...
            var _IS_STANDARD_NORMAL_TEXTURE = FindProperty("_IS_STANDARD_NORMAL_TEXTURE", properties);
            ...
            if (normalTexture.textureValue != null)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    editor.ShaderProperty(_IS_STANDARD_NORMAL_TEXTURE, "Using Standard Normal Texture");

                    // Only show the convert button if we're not using standard normal texture
                    if (_IS_STANDARD_NORMAL_TEXTURE.floatValue == 1)
                    {
                        _IS_STANDARD_NORMAL_TEXTURE.floatValue = 1;
                    }
                }
            }
            else
            ...
            CheckNormalMapSettings(normalTexture, _IS_STANDARD_NORMAL_TEXTURE);
            ...
            if ((currentConfig.Type & (ShaderType.Foliage | ShaderType.UnderwaterFoliage)) != 0)
            {
                ...
                if ((currentConfig.Type & (ShaderType.Foliage | ShaderType.UnderwaterFoliage)) != 0)
                {
                    ...
                    var _USE_WIND = FindProperty("_USE_WIND", properties);
                    editor.ShaderProperty(_USE_WIND, "Enable Wind");
                    GUILayout.Label("Vertex Color R Channel", EditorStyles.miniLabel);

                    var _Individual_Leaf_Offset = FindProperty("_Individual_Leaf_Offset", properties);
                    editor.ShaderProperty(_Individual_Leaf_Offset, "Offset Individual Leaf");
                    GUILayout.Label("Vertex Color b Channel", EditorStyles.miniLabel);


                    GUILayout.Space(10);

                    if (_USE_WIND.floatValue == 1f)
                    {
                        var _HAS_TRUNK = FindProperty("_HAS_TRUNK", properties);
                        editor.ShaderProperty(_HAS_TRUNK, "Enable Trunk Wind");
                        GUILayout.Label("Vertex Color B Channel", EditorStyles.miniLabel);
                    }
            ...
            if (currentConfig.SupportsDetail)
            {
                ...
                var _USE_TRIMSHEET_DETAILS = FindProperty("_USE_TRIMSHEET_DETAILS", properties);
                editor.ShaderProperty(_USE_TRIMSHEET_DETAILS, "Enable Detail Texture Mapping");

                if (_USE_TRIMSHEET_DETAILS.floatValue == 1f)
                {
            ...
            if (currentConfig.Type == ShaderType.UnderwaterNormal)
            {
                var _USE_CAUSTICTS = FindProperty("_USE_CAUSTICTS", properties);
                editor.ShaderProperty(_USE_CAUSTICTS, "Enable Caustics");
