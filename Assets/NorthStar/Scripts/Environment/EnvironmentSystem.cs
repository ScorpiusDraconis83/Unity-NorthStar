...
namespace NorthStar
{
    ...
    public class EnvironmentSystem : Singleton<EnvironmentSystem>
    {
        ...
        [SerializeField] private OceanSimulation oceanSimulation;
        [SerializeField] private QuadtreeRenderer oceanQuadtreeRenderer;
        [SerializeField] private SunDiskRenderer m_SunDiskRenderer;

        [SerializeField] private float transitionTime = 1.0f;
        [SerializeField] private EnvironmentProfile targetProfile;
        ...
        [SerializeField] private WindGustsProfile windGustsProfile;
        ...
        {
            windGustsProfile = profile;
        }
        public void StopWindGusts()
        {
            windGustsProfile = null;
        }
        ...
        private float3 SampleDisplacement(float3 position) //, float time, Vector4 noiseScroll, float noiseScale, float noiseStrength, Texture2D noiseTexture)
        {
            ...
            var displacement = oceanSimulation.DisplacementMap.GetPixelBilinear(oceanUv.x, oceanUv.y);
        ...
        public float GetOceanHeight(float3 position)
        {
            ...
            return oceanSimulation.transform.position.y + oceanSimulation.DisplacementMap.GetPixelBilinear(oceanUv.x - heightUv.r, oceanUv.y - heightUv.b).g;
        }

        public float GetOceanHeightIterative(float3 position, int depth = 4)
        {
            // TODO: get noise displacement working
            //var time = Time.timeSinceLevelLoad;
            //var noiseScroll = CurrentProfile.OceanMaterial.GetVector(s_noiseScroll);
            //var noiseScale = CurrentProfile.OceanMaterial.GetFloat(s_noiseScale);
            //var noiseStrength = CurrentProfile.OceanMaterial.GetFloat(s_noiseStrength);
            //var noiseTexture = CurrentProfile.OceanMaterial.GetTexture(s_noiseTexture) as Texture2D;
            ...
            return height + oceanSimulation.transform.position.y;
        }
        ...
        private void UpdateEnvironment()
        {
            ...
            if (m_sourceProfile == null)
                m_sourceProfile = targetProfile;

            // Check if profile has changed since we last started a transition
            if (targetProfile != m_previousProfile)
            {
                m_lastTransitionStartTime = Time.time;
                m_transitionProfile.StartTransition(targetProfile);
                m_previousProfile = targetProfile;

                m_volumeB.profile = m_volumeA.profile;
                m_volumeA.profile = targetProfile.PostProcessProfile == null ? m_defaultVolumeProfile : targetProfile.PostProcessProfile;
            }

            // Update profile
            var theTransitionTime = m_oneOffTransitionTime ?? transitionTime;
            ...
            if (t < 1.0f)
            {
                ...
                m_transitionProfile.Lerp(m_sourceProfile, targetProfile, t);
            ...
            if (oceanSimulation.isActiveAndEnabled)
            {
                oceanSimulation.Profile = CurrentProfile;
                oceanSimulation.UpdateSimulation(WindVector);
            }

            oceanQuadtreeRenderer.Material = CurrentProfile.OceanMaterial;
        ...
        private void BeginContextRendering(ScriptableRenderContext context, List<Camera> cameras)
        {
            ...
            if (oceanSimulation.isActiveAndEnabled)
            {
                oceanSimulation.BeginContextRendering();
                ...
                m_oceanPropertyBlock.SetTexture(s_oceanDisplacement, oceanSimulation.DisplacementMap);
                m_oceanPropertyBlock.SetTexture(s_oceanNormal, oceanSimulation.NormalMap);
            ...
            var oceanTexelsPerMeter = oceanSimulation.Resolution / CurrentProfile.OceanSettings.PatchSize;
            if (oceanQuadtreeRenderer.isActiveAndEnabled)
                oceanQuadtreeRenderer.BeginContextRendering(cameras, m_oceanPropertyBlock, oceanTexelsPerMeter);
