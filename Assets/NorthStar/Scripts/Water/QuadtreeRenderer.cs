...
namespace NorthStar
{
    ...
    public class QuadtreeRenderer : MonoBehaviour
    {
        ...
        private float cullingBoundScale = 1.5f;

        [SerializeField] private float lodThreshold = 1.5f;

        [SerializeField] private float skirtingSize = 10000f;
        ...
        private int LodMapWidth => 1 << lodLevels; // 2^lodLevels
        private int LodMapResolution => 1 << (lodLevels << 1); // (2^x)^2
        ...
        public void BeginContextRendering(List<Camera> cameras, MaterialPropertyBlock propertyBlock, float displacementTexelsPerMeter)
        {
            ...
            foreach (var camera in cameras)
            {
                ...
                if (m_updateQuadtree)
                {
                    ...
                    var cellSize = size / vertexCount / 2;
                    ...
                    new CheckQuadtreeJob(cullingBoundScale, maxHeight, lodThreshold, size, lodLevels, LodMapWidth, m_currentGridCenter, camera.transform.position, m_frustumPlanesNative, m_visiblePatches, m_subdivisionMap).Run();
                    Profiler.EndSample();
        ...
        {
            var size = vertexCount + 1;
            var vertices = new Vector3[size * size];
            var xDelta = 1f / vertexCount;
            var yDelta = 1f / vertexCount;
            ...
            for (var i = 0; i < SKIRTINGSEGMENTS; ++i)
            {
                ...
                skirtingVerts[i * 2 + 0] = direction * this.size / 2f;
                skirtingVerts[i * 2 + 1] = direction * skirtingSize / 2f;
            }
        ...
        {
            var stepSize = 1 << (lodLevels - lod); // 2^(x-y)
            var x = x0 * stepSize;
            var y = y0 * stepSize;
            ...
            {
                var patchSize = size / (1 << lod);

                Vector3 center;
                center.y = 0;
                center.x = m_currentGridCenter.x - 0.5f * size + (x0 + 0.5f) * patchSize;
                center.z = m_currentGridCenter.z - 0.5f * size + (y0 + 0.5f) * patchSize;
