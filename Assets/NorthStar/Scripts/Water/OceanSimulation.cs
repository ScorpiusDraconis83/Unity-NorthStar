...
using static NorthStar.MathUtils;
...
namespace NorthStar
{
    ...
    public class OceanSimulation : MonoBehaviour
    {
        ...
        private bool enableEditorUpdates = false;
        ...
        {
            m_dispersionTable = new NativeArray<float>(resolution * resolution, Allocator.Persistent);
            m_spectrum = new NativeArray<float4>(resolution * resolution, Allocator.Persistent);

            DisplacementMap = new Texture2D(resolution, resolution, TextureFormat.RGBAHalf, true);
            NormalMap = new Texture2D(resolution, resolution, TextureFormat.RGBA32, true, true) { filterMode = FilterMode.Trilinear };
            ...
            var mipCount = (int)Mathf.Log(resolution, 2) + 1;
            m_normalResult = new NativeArray<float4>[mipCount];

            for (var i = 0; i < mipCount; i++)
            {
                var mipSize = resolution >> i;
        ...
        public void UpdateSimulation(Vector3 windVector)
        {
            ...
            var resolutionChanged = resolution != m_cachedResolution;
            ...
            if (m_cachedSettingsVersion != Profile.Version || resolutionChanged || m_cachedProfile != Profile || m_cachedWindVector != windVector)
            {
                ...
                var spectrumJob = new OceanSpectrumJob(Profile.OceanSettings.Directionality, Profile.OceanSettings.Gravity, windSpeed, Profile.OceanSettings.MinWaveSize, Profile.OceanSettings.PatchSize, Profile.OceanSettings.SequenceLength, resolution, windDirection, m_dispersionTable, m_spectrum);
                m_jobHandle = spectrumJob.Schedule(resolution * resolution, 64, m_jobHandle);
            ...
            if (!forceUpdate && !Application.isPlaying && !enableEditorUpdates)
                return;

            // Calculate the iFFT
            var length = Square(resolution);

            var dispersion = new OceanDispersionJob(m_dispersionTable, m_spectrum, m_heightBufferB, m_displacementBufferB, resolution, Time.time * Profile.OceanSettings.TimeScale);
            ...
            {
                // Gather across rows
                var fftJob = new OceanFFTRowJob(resolution, passes, m_butterflyLookupTable,
                    heightBuffers[~j & 1], displacementBuffers[~j & 1],
                    heightBuffers[j & 1], displacementBuffers[j & 1]);
                m_jobHandle = fftJob.Schedule(resolution, 1, m_jobHandle);
                j += passes;
            }

            {
                ...
                var fftJob = new OceanFFTColumnJob(resolution, passes - 1, m_butterflyLookupTable,
                    heightBuffers[~j & 1], displacementBuffers[~j & 1],
                    heightBuffers[j & 1], displacementBuffers[j & 1]);
                m_jobHandle = fftJob.Schedule(resolution, 1, m_jobHandle);
                j += 1;
...

            {
                ...
                var finalJob = new OceanFFTFinalJob(resolution, passes - 1, Profile.OceanSettings.Choppyness, m_butterflyLookupTable,
                    heightBuffers[~j & 1], displacementBuffers[~j & 1],
            ...
            var normalFoldJob = new OceanNormalFoldingJob(resolution, Profile.OceanSettings.PatchSize, smoothness, m_displacementResult, normalPixels, m_normalResult[0]);
            ...
            {
                var mipResolution = resolution >> i;
                var pixelCount = (4 * resolution * resolution - 1) / 3;
        ...
        {
            return (int)(reversebits((uint)i) >> (lzcnt(resolution) + 1));
        }

        private void ComputeButterflyLookupTable()
        {
            var passes = (int)Mathf.Log(resolution, 2);
            m_butterflyLookupTable = new(resolution * passes, Allocator.Persistent);
            ...
            for (var i = 0; i < passes; i++)
            {
                ...
                for (var j = 0; j < nBlocks; j++)
                {
                    for (var k = 0; k < nHInputs; k++)
                    {
                        ...
                        var wr = Mathf.Cos(2.0f * Mathf.PI * (k * nBlocks) / resolution);
                        var wi = Mathf.Sin(2.0f * Mathf.PI * (k * nBlocks) / resolution);

                        var offset1 = i1 + i * resolution;
                        m_butterflyLookupTable[offset1] = (new(j1, j2), new(wr, wi));

                        var offset2 = i2 + i * resolution;
